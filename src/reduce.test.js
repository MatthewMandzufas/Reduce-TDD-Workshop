...
describe("Reduce", () => {
  it("Should reduce an array of numbers to their sum", () => {
    ...
    it("Should use the initial value as the accumulator instead of the first element of the array", () => {
      ...
      const sumOfNumbersInAnArray = reduce(
        arrayToReduce,
        reducer,
        initialValue,
      );
    ...
    it("Should only operate on an array of numbers", () => {
      const arrayToReduce = [1, 2, undefined, 5, 6];
      function reducer(accumulator, currentItem) {
        return accumulator + currentItem;
      }
      expect(() => reduce(arrayToReduce, reducer)).toThrow(
